...
public class GameGUI extends JComponent {
   ...
   * Creates a frame with a background image and a player that will move around
   * the board.
   */
  public GameGUI() {
  ...
  /**
   * After a GameGUI object is created, this method adds the walls, prizes, and
   * traps to the gameboard.
   ...
   * <P>
   * precondition: amount to move is not larger than the board, otherwise player
   * may appear to disappear
   * postcondition: increases number of steps even if the player did not actually
   * move (e.g. bumping into a wall)
   * <P>
   * 
   * @param incrx amount to move player in x direction
   * @param incry amount to move player in y direction
   * @return penalty score for hitting a wall or potentially going off the grid, 0
   *         otherwise
   */
  public int movePlayer(int incrx, int incry) {
    ...
    for (Rectangle r : walls) {
      ...
      // (Note: the following if statements could be written as huge conditional but
  ...
  /**
   * Check the space adjacent to the player for a trap. The adjacent location is
   * one space away from the player,
   * designated by newx, newy.
   * <P>
   * precondition: newx and newy must be the amount a player regularly moves,
   * otherwise an existing trap may go undetected
   * <P>
   * 
   * @param newx a location indicating the space to the right or left of the
   *             player
   * @param newy a location indicating the space above or below the player
   * @return true if the new location has a trap that has not been sprung, false
   *         otherwise
   */
  ...
  public void checkTrap(int newx, int newy) {
    playerSteps++;
    if (!isTrap(newx, newy)) {
      System.out.println("No trap has been found.");
    } else {
   ...
   * <P>
   * precondition: newx and newy must be the amount a player regularly moves,
   * otherwise an existing trap may go unsprung
   * <P>
   * 
   * @param newx a location indicating the space to the right or left of the
   *             player
  ...
  /**
   * Pickup a prize and score points. If no prize is in that location, this
   * results in a penalty.
   * <P>
   * 
   * @return positive score if a location had a prize to be picked up, otherwise a
   *         negative penalty
   */
  ...
  /**
   * Set the designated number of prizes in the game. This can be used to
   * customize the gameboard configuration.
   * <P>
  ...
  /**
   * Set the designated number of traps in the game. This can be used to customize
   * the gameboard configuration.
   * <P>
  ...
  /**
   * Set the designated number of walls in the game. This can be used to customize
   * the gameboard configuration.
   * <P>
   * precondition t must be a positive, non-zero integer
   * <P>
  ...
  /**
   * Reset the board to replay existing game. The method can be called at any time
   * but results in a penalty if called
  ...
  /**
   * For internal use and should not be called directly: Users graphics buffer to
   * paint board elements.
   */
  ...
  /*
   * Add randomly placed traps to the board. They will be painted white and appear
   * invisible.
   * Note: prizes and traps may occupy the same location, with traps hiding prizes
   */
